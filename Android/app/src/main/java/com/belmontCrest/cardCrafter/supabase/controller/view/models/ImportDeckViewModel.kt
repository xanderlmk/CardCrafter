        ...
        viewModelScope.launch {
            try {
                ...
                if (ctd.second == SUCCESS) {
                    _cardsToDisplay.update { ctd.first }.also {
                        _theCards.update { importRepository.getCards(ctd.first) }
                    }
                } else {
                    _errorMessage.update { "Failed to get cards" }
                }
            } catch (e: Exception) {
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                supabaseToRoomRepository.replaceDeckList(
                    ...
                    onProgress = { onProgress(it) }, cardObject.total
                )
...
private fun returnError(e: Exception, onError: (String) -> Unit): Pair<Int, String> = when (e) {
    is SocketException -> {
