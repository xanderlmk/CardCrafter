...
class CoOwnerViewModel(
    ...
    fun acceptRequest(uuid: String) {
        viewModelScope.launch {
            ...
            when (result) {
                ReturnValues.SUCCESS -> { _status.update { RequestStatus.Accepted }; getRequests() }
                ReturnValues.NETWORK_ERROR -> _status.update { RequestStatus.Error("Network Error") }
                ReturnValues.CANCELLED -> _status.update { RequestStatus.Error("Request was cancelled") }
                ReturnValues.UNKNOWN_ERROR -> {
    ...
    fun declineRequest(uuid: String) {
        viewModelScope.launch {
            ...
            when (result) {
                ReturnValues.SUCCESS -> { _status.update { RequestStatus.Declined }; getRequests() }
                ReturnValues.NETWORK_ERROR -> _status.update { RequestStatus.Error("Network Error") }
                ReturnValues.CANCELLED -> _status.update { RequestStatus.Error("Request was cancelled") }
                ReturnValues.UNKNOWN_ERROR -> {
                    _status.update { RequestStatus.Error("Unknown Error") }
