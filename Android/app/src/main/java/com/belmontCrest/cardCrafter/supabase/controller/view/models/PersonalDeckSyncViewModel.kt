...
class PersonalDeckSyncViewModel(
        ...
        viewModelScope.launch {
            try {
                ...
                if (syncInfo != null) {
                    ...
                    if (remoteSync.second != ReturnValues.SUCCESS) {
    ...
    private fun validateRemoteSync(remoteSync: Pair<PDUpdatedOn?, Int>): Boolean {
        if (remoteSync.second != ReturnValues.SUCCESS || remoteSync.first == null) {
            val errorMessage = if (remoteSync.second != ReturnValues.SUCCESS) "Error" else "Empty updated_on"
