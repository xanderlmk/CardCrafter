...
val MIGRATION_9_10 = object : Migration(9, 10) {
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        try {
            ...
            db.execSQL("ALTER TABLE cards ADD COLUMN deckUUID TEXT NOT NULL DEFAULT ''")

            // Add the reviewAmount column to the decks table
            db.execSQL("ALTER TABLE decks ADD COLUMN reviewAmount INTEGER NOT NULL DEFAULT 1")

            // Add the reviewsLeft column to the cards table
            db.execSQL("ALTER TABLE cards ADD COLUMN reviewsLeft INTEGER NOT NULL DEFAULT 1")

            // Generate UUIDs for existing decks
            val cursorDecks = db.query("SELECT id FROM decks")
            while (cursorDecks.moveToNext()) {
                val id = cursorDecks.getInt(cursorDecks.getColumnIndexOrThrow("id"))
                val uuid = UUID.randomUUID().toString()
                db.execSQL("UPDATE decks SET uuid = '$uuid' WHERE id = $id")
            }
            cursorDecks.close()

            // Generate deckUUIDs for existing cards
            val cursorCards = db.query("SELECT id, deckId FROM cards")
            while (cursorCards.moveToNext()) {
                ...
                // Get the deck's UUID
                val cursorDeck = db.query("SELECT uuid FROM decks WHERE id = $deckId")
                if (cursorDeck.moveToFirst()) {
                    val deckUUID = cursorDeck.getString(cursorDeck.getColumnIndexOrThrow("uuid"))
                    db.execSQL("UPDATE cards SET deckUUID = '$deckUUID' WHERE id = $id")
                }
                cursorDeck.close()
            }
            cursorCards.close()
