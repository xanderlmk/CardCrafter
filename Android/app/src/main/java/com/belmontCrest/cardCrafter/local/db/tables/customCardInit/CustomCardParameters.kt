...
import com.belmontCrest.cardCrafter.local.db.tables.toParcelableMiddleParam
import com.belmontCrest.cardCrafter.local.db.tables.toParcelableParam
...
sealed class Param : Parcelable {
    ...
    companion object : Parceler<Param> {
        const val STRING_LIST = "StringList"
        ...
        override fun Param.write(parcel: Parcel, flags: Int) {
            when (this) {
                is Pair -> {
                    ...
                    parcel.writeParamType(this.first)
                    parcel.writeParamType(this.second)
                }

                is Type -> {
                    parcel.writeByte(1.toByte())
                    parcel.writeParamType(this)
                }
            }
        }

        override fun create(parcel: Parcel): Param {
            return parcel.toParcelableParam()!!
        }
    }
}

private fun Parcel.writeParamType(key: String, value: String) {
    this.writeString(key)
    this.writeString(value)
}

private fun Parcel.writeParamType(param: Param.Type) {
    when (param) {
        is Param.Type.Audio -> {
            this.writeParamType(Param.AUDIO, param.uri)
        }

        is Param.Type.Image -> {
            this.writeParamType(Param.IMAGE, param.uri)
        }

        is Param.Type.Notation -> {
            this.writeParamType(Param.NOTATION, param.s)
        }

        is Param.Type.String -> {
            this.writeParamType(Param.STRING, param.s)
        }

        is Param.Type.StringList -> {
            this.writeString(Param.STRING_LIST)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
                this.writeList(param.list)
            else {
                val b = Bundle()
                b.putStringArrayList("list", ArrayList(param.list))
                this.writeBundle(b)
            }
        }
    }
...
sealed class MiddleParam : Parcelable {
    ...
    companion object: Parceler<MiddleParam> {
        const val EMPTY = "Empty"
        const val HINT = "Hint"
        const val WITH_PARAM = "WithParam"
        override fun MiddleParam.write(parcel: Parcel, flags: Int) {
            when (this) {
                is Empty -> {
                ...
                is WithParam -> {
                    parcel.writeString(WITH_PARAM)
                    param.write(parcel, flags)
                }
            }
        }
        override fun create(parcel: Parcel): MiddleParam {
            return parcel.toParcelableMiddleParam()!!
        }
    }
}
...
sealed class AnswerParam : Parcelable {
    ...
    @Serializable
    @SerialName("$ANSWER_PARAM.Choice")
    @Parcelize
    data class Choice(val choices: List<String>, val correct: Char) : AnswerParam()
